  ...
  return (
    <div className='relative lg:w-1/5 w-full'>
      ...
      <div className={display + " lg:hidden flex flex-col rounded-md absolute  top-0 left-3 right-3 bg-gray-300"}>
        ...
        <Filter ChangeP={ChangeP} NavTO={NavTO} />
      </div>
    </div>
  )
}
const Filter = ({ ChangeP, NavTO }) => {
  ...
  return (
    <div className="books flex flex-col mb-2 px-4">
      ...
      <div className="flex flex-col pl-2">
        <div className="min">
          <label>Min : {min}</label>
          <input type="range" min={0} max={500} onChange={(e) => setMin(e.target.value)} defaultValue={min} />
        </div>
        <div className="min">
          <label>Max : {max}</label>
          <input type="range" min={600} max={1200} onChange={(e) => setMax(e.target.value)} defaultValue={max} />
        </div>
        <button onClick={() => changePrice()} className='border-2 bg-gray-300 mx-8 mt-1'>Apply</button>
      </div>
    </div>
  )
}
